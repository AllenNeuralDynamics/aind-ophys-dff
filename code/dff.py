...
def plot_dff(
    ...
    """Per-ROI QC plot: writes ``<fig_path>/<unique_id>_<roi_id>_dff.png``.

    Thin wrapper around ``aind_ophys_utils.dff.plot_dff``: builds the
    time vector from ``frame_rate``, delegates rendering to utils, then
    appends triexp diagnostics to the title (when ``log`` is provided)
    and saves the figure with the capsule's filename convention.

    Parameters
    ----------
    trace : NDArray
        Neuropil-corrected raw fluorescence trace, shape ``(T,)``.
    baseline : NDArray
        Fitted baseline F0, shape ``(T,)``.
    frame_rate : float
        Frame rate in Hz; used to construct the time axis.
    roi_id : int
        ROI identifier (used in title + filename).
    fig_path : str or Path
        Directory where the output PNG is saved.
    unique_id : str
        Experiment ID prefix for the filename.
    log : dict, optional
        Per-ROI triexp log dict (one element of ``compute_dff``'s ``logs``).
        When provided, the plot title gains a
        ``(n_passes=…, winner=…, trigger=…)`` suffix. ``None`` for the
        percentile path. Default ``None``.
    zoom_duration : float, optional
        Length of zoomed inset windows in seconds. ``None`` or ``0``
        disables insets. Default 60.0.
    """
    t = np.arange(len(trace)) / frame_rate
    fig = dff_percentile.plot_dff(
        F=trace, F0=baseline, t=t,
        zoom_duration=zoom_duration, roi_id=roi_id,
    )
    if log is not None:
        title = (
            f"cell_roi_id: {int(roi_id)}  "
